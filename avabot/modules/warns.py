...
from avabot import dispatcher, BAN_STICKER
from avabot.modules.disable import DisableAbleCommandHandler
from avabot.modules.helper_funcs.chat_status import (is_user_admin, bot_admin, user_admin_no_reply, user_admin,
                                                     can_restrict)
from avabot.modules.helper_funcs.extraction import extract_text, extract_user_and_text, extract_user
from avabot.modules.helper_funcs.filters import CustomFilters
from avabot.modules.helper_funcs.misc import split_message
from avabot.modules.helper_funcs.string_handling import split_quotes
from avabot.modules.log_channel import loggable
from avabot.modules.sql import warns_sql as sql
...
def set_warn_strength(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            return (f"<b>{html.escape(chat.title)}:</b>\n"
                    f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
                    f"Has enabled strong warns. Users will be seriously kicked.")
            ...
            return (f"<b>{html.escape(chat.title)}:</b>\n"
                    f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
                    f"Has disabled strong kicks. I will use normal kicks on users.")
